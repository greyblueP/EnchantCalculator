    ...
    for i in Enchanting:
        if i == "冲突附魔":
            ...
            for j in Enchanting[i]["适用装备"]:
                if j not in data:
                    data[j] = []
                data[j].append([Enchanting[i]["魔咒"], Enchanting[i]["最高等级"], Enchanting[i]["权重"]])
...
def function(input_equip, input_enchant):
    ...
    # 第一次 1
    print(f"{str(euqip[1]+1)} 【{data[0][1]}】{euqip[0]} + {data[0][0]}")
    euqip[2] += data[0][1]
    ...
    # 第二次 2
    if len(data) > 1:  # 2+
        print(f"{str(euqip[1]+1)} 【{data[-1][1]}】{data[0][0]} + {data[-1][0]} = 附魔书")
        euqip[2] += data[-1][1]
    ...
    # 第三次 4
    if len(data) > 3:  # 4+
        print(f"{str(euqip[1]+1)} 【{data[-1][1]}】{data[0][0]} + {data[-1][0]} = 附魔书1")
        euqip[2] += data[-1][1]
